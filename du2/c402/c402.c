...
**/
	...
	tBTNodePtr rootCopy = (*RootPtr);

	// Vyhľadanie, či už existuje uzol s daným Content
	if (rootCopy == NULL) {
		found = false;
		where = NULL;
	}
		...
		while (rootCopy != NULL) {
			where = rootCopy; // Uchovanie hodnoty where
			if (rootCopy->Cont > Content) { // Posun vĺavo
				rootCopy = rootCopy->LPtr;
			}
			else if (rootCopy->Cont < Content) { // Posun vpravo
				rootCopy = rootCopy->RPtr;
			}
			else {
				found = true;
				break;
	...
	if (found) {
		...
		tBTNodePtr newPtr = (tBTNodePtr) malloc(sizeof(struct tBTNode));
		if (newPtr != NULL) {
		...
		if (where == NULL) {
			// Nový koreň do prázdneho stromu
			(*RootPtr) = newPtr;
		}
		else {
			// Nový sa pripojí ako list
...
**/
	...
	SInitP(&stackP);
	do {
		if ((*RootPtr) == NULL) { // zoberiem uzol zo zásobníku
			if (SEmptyP(&stackP) == false) {
				(*RootPtr) = STopPopP(&stackP);
			}
		}
		else { // Pravého dám do zásobníka
			if ((*RootPtr)->RPtr != NULL) {
				SPushP(&stackP, (*RootPtr)->RPtr);
			}
			tBTNodePtr temp = (*RootPtr);
			(*RootPtr) = (*RootPtr)->LPtr; // Pohyb doľava
			free(temp); // Zrušiť aktuálny uzol
		}
	} while ((*RootPtr) != NULL || (SEmptyP(&stackP) != true));
