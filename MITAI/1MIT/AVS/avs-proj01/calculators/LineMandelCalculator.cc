	...
	BaseMandelCalculator(matrixBaseSize, limit, "LineMandelCalculator")
{
    ...
    zReal_arr = (float *)_mm_malloc(width * sizeof(float), 64);
    zImg_arr = (float *)_mm_malloc(width * sizeof(float), 64);
...
int * LineMandelCalculator::calculateMandelbrot () {
	...
	for (int i = 0; i < height; i++) { // go over all rows in result matrix
		...
		for (int a = 0; a < width; a++) {
            *(p_zImg + a) = y;
            *(p_zReal + a) = x_start + a * dx;
		}

		for (int k = 0; k < limit; k++) { // go over that row limit times
			int sum = 0;

			#pragma omp simd aligned(pdata, p_zImg, p_zReal: 64) reduction(+:sum)
			for (int j = 0; j < width; j++) { // go over all cols in the row
                ...
                sum += (r2 + i2 > 4.0f) ? 1 : 0;
                *(pdata + i * width + j) = (r2 + i2 > 4.0f) ? std::min(*(pdata + i * width + j), k) : *(pdata + i * width + j);

                *(p_zImg + j) = (r2 + i2 < 4.0f) ? 2.0f * real * imag + y : imag;
                *(p_zReal + j) = (r2 + i2 < 4.0f) ? r2 - i2 + x : real;
			}

			if (sum == width) { // all blown up - no need for more iterations over this row
				break;
			}
		}
	}
